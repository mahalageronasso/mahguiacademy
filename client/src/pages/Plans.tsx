...
export default function Plans() {
  ...
  const MENTORSHIP = "https://tally.so/r/QK24jA";
  const plans: Plan[] = [
          ...
          <div className="container">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 max-w-6xl mx-auto">
              {plans.map((plan, i) => {
                ...
                const isMentorship = plan.accent === "#1a1a1a";
                const href = isMentorship ? MENTORSHIP : GUMROAD;
                return (
                  <motion.div key={plan.nameKey} initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.1 }}
                    ...
                    <div className="p-6 lg:p-7">
                      <div className="w-10 h-10 rounded-lg flex items-center justify-center mb-4" style={{ backgroundColor: isMentorship ? "#f3f4f6" : plan.accent + "18", color: plan.accent }}>
                        {plan.icon}
                      </div>
                      ...
                      <ul className="space-y-3 mb-8">
                        {plan.features.map((f) => (
                          <li key={f.key} className="flex items-start gap-2.5">
                            <div className="w-4 h-4 rounded-full flex items-center justify-center mt-0.5 shrink-0" style={{ backgroundColor: isMentorship ? "#f3f4f6" : plan.accent + "18" }}>
                      ...
                      <a href={href} target="_blank" rel="noopener noreferrer"
                        ...
                        style={isHighlighted ? { backgroundColor: plan.accent } : { borderColor: isMentorship ? "#d1d5db" : plan.accent, color: isMentorship ? "#374151" : plan.accent }}>
                        {t("plans.cta")}
                      </a>
                    </div>
                  </motion.div>
