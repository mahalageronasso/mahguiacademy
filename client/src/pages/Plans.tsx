...
interface Plan {
  nameKey: string; descKey: string; priceKey: string; perKey: string;
  brlKey: string; features: PlanFeature[]; accent: string;
  popular?: boolean; badgeKey?: string; icon: React.ReactNode;
}
                      ...
                      <ul className="space-y-3 mb-8">
                        {plan.features.map((f) => (
                          <li key={f.key} className="flex items-start gap-2.5">
                            <div className="w-4.5 h-4.5 rounded-full flex items-center justify-center mt-0.5 shrink-0" style={{ backgroundColor: plan.accent === "#1a1a1a" ? "#f3f4f6" : plan.accent + "18" }}>
